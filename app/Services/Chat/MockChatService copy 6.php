...
class MockChatService implements ChatServiceInterface
{
    ...
    public function reply(string $userId, string $lastUserMessage, ?string $customer = null): string
    {
        ...
        if ($state === 'awaiting_supplier_piva') {
            // Se è una P.IVA valida (11 cifre) → aggiungi
            if (preg_match($vatRegex, $msg)) {
                $user = User::where('phone', $customer)->first();
                if (!$user) {
                    Cache::put($this->stateKey($userId), 'awaiting_piva', now()->addHours(12));
                ...
                try {
                    ...
                    // normalizza stato attuale a array
                    ...
                    // 1) assicurati che 'fornitori' sia un array
                    $usersColl->updateOne(['phone' => $customer], [
                        '$set' => ['fornitori' => array_values(array_unique($initial))],
                    ]);

                    // 2) aggiungi in modo unico la nuova P.IVA
                    $usersColl->updateOne(['phone' => $customer], [
                        '$addToSet' => ['fornitori' => (string) $msg],
                    ]);

                    // rimani nello stato per un’eventuale altra aggiunta
            ...
            // Se l'utente risponde esplicitamente "sì/ok/yes" → chiedi un’altra P.IVA
            if ($isYes($msg)) {
                return $say("Perfetto! Inviami la **P.IVA (11 cifre)** del nuovo fornitore.");
            }

            // Tutto ciò che NON sono 11 cifre → trattalo come **NO**
            Cache::put($this->stateKey($userId), 'ready_for_order', now()->addHours(12));
